...
import com.ipsoflatus.dreamgifts.modelo.dao.exceptions.IllegalOrphanException;
...
public class CategoriaArticuloJpaController implements Serializable {
    ...
    public void edit(CategoriaArticulo categoriaArticulo) throws IllegalOrphanException, NonexistentEntityException, Exception {
        EntityManager em = null;
        try {
            em = getEntityManager();
            em.getTransaction().begin();
            ...
            for (Articulo articulosOldArticulo : articulosOld) {
                if (!articulosNew.contains(articulosOldArticulo)) {
                    ...
                    illegalOrphanMessages.add("You must retain Articulo " + articulosOldArticulo + " since its categoriaArticulo field is not nullable.");
                }
            }
            if (illegalOrphanMessages != null) {
    ...
    public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException {
        ...
        try {
            ...
            List<String> illegalOrphanMessages = null;
            List<Articulo> articulosOrphanCheck = categoriaArticulo.getArticulos();
            for (Articulo articulosOrphanCheckArticulo : articulosOrphanCheck) {
                if (illegalOrphanMessages == null) {
                    illegalOrphanMessages = new ArrayList<String>();
                }
                illegalOrphanMessages.add("This CategoriaArticulo (" + categoriaArticulo + ") cannot be destroyed since the Articulo " + articulosOrphanCheckArticulo + " in its articulos field has a non-nullable categoriaArticulo field.");
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            }
